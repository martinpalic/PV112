...
void main()
{
    ...
    vec3 spot_light_vector = spotlight.position.xyz - fs_position;
    vec3 spot_light_vector_norm = -normalize(spot_light_vector);
    float theta = dot(normalize(spotlight.direction.xyz), spot_light_vector_norm);
	theta = max(theta, 0);
	
    if(acos(theta) < spotlight.cos_cutoff)
    {
		if((fs_texture_coordinate.x < 0.40) && (fs_texture_coordinate.y < 0.40) ){
			color_sum = mix(vec3(255.0 / 255.0,255.0 / 255.0,120 / 255.0), color_sum, 0.4); //pow(length(spot_light_vector), 2));
			}
    }
